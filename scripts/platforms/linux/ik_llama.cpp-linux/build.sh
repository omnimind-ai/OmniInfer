...
PACKAGE_ROOT="${REPO_ROOT}/.local/runtime/linux/ik_llama.cpp-linux"
LLAMA_ROOT="${REPO_ROOT}/framework/ik_llama.cpp"
BUILD_ROOT="${PACKAGE_ROOT}/build/ik_llama.cpp-linux"
BIN_ROOT="${PACKAGE_ROOT}/bin"
LOG_ROOT="${PACKAGE_ROOT}/logs"
MODELS_ROOT="${REPO_ROOT}/.local/models"

if [[ ${BUILD_FROM_SOURCE} -eq 0 ]]; then
  echo "No prebuilt install path is configured for ik_llama.cpp-linux." >&2
...
echo "Configuring ik_llama.cpp Linux CPU build..."
...
echo
echo "ik_llama.cpp Linux CPU build complete."
echo "Binary package location: ${BIN_ROOT}"
echo "Models directory: ${MODELS_ROOT}"
echo "Next step:"
echo "  ./omniinfer backend select ik_llama.cpp-linux"
