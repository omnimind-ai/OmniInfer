...
$InnerScript = Join-Path $PlatformRoot "llama.cpp-windows-arm64\build.ps1"

if (-not (Test-Path -LiteralPath $InnerScript)) {
    throw "Windows arm64 build script not found: $InnerScript"
}
...
Write-Host "Running Windows arm64 backend build script:"
