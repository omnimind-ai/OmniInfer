...
def load_tui_show_reasoning(app_root: Path | None = None) -> bool:
    value = load_state(app_root).get("tui_show_reasoning")
    if isinstance(value, bool):
        return value
    if isinstance(value, str):
        lowered = value.strip().lower()
        if lowered in {"on", "true", "1", "yes", "show", "shown", "visible"}:
            return True
        if lowered in {"off", "false", "0", "no", "hide", "hidden"}:
            return False
    return False


def save_tui_show_reasoning(enabled: bool, app_root: Path | None = None) -> None:
    payload = load_state(app_root)
    payload["tui_show_reasoning"] = "on" if enabled else "off"
    save_state(payload, app_root)
