...
def _mlx_fake_modules(test_case: unittest.TestCase) -> dict[str, types.ModuleType]:
    ...
    def lm_generate(model, tokenizer_obj, prompt, max_tokens, sampler=None, verbose=False, **kwargs):
        test_case.assertEqual(model.config.model_type, "llama")
        test_case.assertIs(tokenizer_obj, tokenizer)
        test_case.assertIn("user: hello", prompt)
        test_case.assertEqual(max_tokens, 32)
        test_case.assertEqual(sampler, {"temp": 0.5, "top_p": 0.9})
        test_case.assertEqual(kwargs.get("prefill_step_size"), 512)
        test_case.assertEqual(kwargs.get("kv_bits"), 4)
        test_case.assertEqual(kwargs.get("kv_group_size"), 32)
        test_case.assertEqual(kwargs.get("quantized_kv_start"), 256)
        test_case.assertFalse(verbose)
        return "hello world"
    ...
    def vlm_apply_chat_template(processor_obj, config, messages, add_generation_prompt=True, num_images=0):
        test_case.assertIs(processor_obj, processor)
        test_case.assertEqual(config.model_type, "qwen2_vl")
        test_case.assertTrue(add_generation_prompt)
        test_case.assertEqual(num_images, 1)
        test_case.assertEqual(messages[0]["role"], "user")
        return "vision prompt"

    def vlm_generate(model, processor_obj, prompt, image=None, max_tokens=128, verbose=False, **kwargs):
        test_case.assertEqual(model.config.model_type, "qwen2_vl")
        test_case.assertIs(processor_obj, processor)
        test_case.assertEqual(prompt, "vision prompt")
        test_case.assertEqual(image, "data:image/png;base64,AAA")
        test_case.assertEqual(max_tokens, 24)
        test_case.assertEqual(kwargs.get("temperature"), 0.3)
        test_case.assertEqual(kwargs.get("top_p"), 0.8)
        test_case.assertEqual(kwargs.get("prefill_step_size"), 1024)
        test_case.assertEqual(kwargs.get("kv_bits"), 8)
        test_case.assertEqual(kwargs.get("kv_group_size"), 64)
        test_case.assertEqual(kwargs.get("quantized_kv_start"), 0)
        test_case.assertFalse(verbose)
...
def _mnn_fake_modules(test_case: unittest.TestCase, fake_cv: _FakeCvModule) -> dict[str, types.ModuleType]:
    llm_module = types.ModuleType("MNN.llm")

    def create(config_path):
        test_case.assertTrue(str(config_path).endswith("config.json"))
        return _FakeLlm()
...
class MlxDriverTests(unittest.TestCase):
    ...
    def test_text_chat_completion_and_streaming(self) -> None:
        ...
        with patch.dict(sys.modules, _mlx_fake_modules(self), clear=False):
...
class MnnDriverTests(unittest.TestCase):
    ...
    def test_text_chat_completion_and_streaming(self) -> None:
        ...
        with patch.dict(sys.modules, _mnn_fake_modules(self, fake_cv), clear=False):
        ...
        self.assertEqual(response["usage"]["prompt_tokens"], 3)  # tokenizer_encode splits on whitespace
    ...
    def test_multimodal_chat_completion_and_streaming(self) -> None:
        ...
        self.assertEqual(response["usage"]["prompt_tokens"], 3)  # tokenizer_encode splits prompt_text on whitespace
        self.assertEqual(response["usage"]["completion_tokens"], 2)
        self.assertIn("predicted_per_second", response["timings"])
