...
sources=(
    "${repo_root}/android/omniinfer-server/src/main/java/com/omniinfer/server/OmniInferEngineManifest.kt"
    "${repo_root}/android/omniinfer-server/src/test/kotlin/com/omniinfer/server/OmniInferEngineManifestTest.kt"
)
if [[ -n "${KOTLIN_COMPILER_CLASSPATH:-}" ]]; then
    ...
    java -cp "${test_root}/tests.jar:${KOTLIN_COMPILER_CLASSPATH}" com.omniinfer.server.OmniInferEngineManifestTestKt
else
