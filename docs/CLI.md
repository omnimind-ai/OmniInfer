...
- The TUI chat commands are `/backend`, `/model`, `/status`, `/clear`, `/help`, and `/exit`.
...
- TUI `/status` shows the current backend, loaded model, context size, request defaults, conversation message count, and conversation context usage. For llama.cpp-compatible backends, the TUI reads backend `/props` when the load-time `ctx-size` was not explicit. Context usage is shown after a chat response returns a `usage` payload; it is calculated from the last full-history response's `total_tokens` against the loaded `ctx-size`.
