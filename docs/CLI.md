...
- The TUI chat commands are `/backend`, `/model`, `/think`, `/status`, `/clear`, `/help`, and `/exit`. Use `/think` to toggle the gateway default thinking mode, or `/think on` / `/think off` to set it explicitly. The thinking choice is saved under `.local/config/state.json` and reused the next time the gateway starts.
- The TUI keeps a fixed bottom input bar during chat and while responses stream so the conversation history stays visible above the prompt. The prompt area also shows a compact status line with the active backend, loaded model, thinking mode, recent context usage, and transient notices from backend/model/thinking commands.
...
- TUI `/status` shows the current backend, loaded model, context size, request defaults, conversation message count, and conversation context usage. For llama.cpp-compatible backends, the TUI reads backend `/props` when the load-time `ctx-size` was not explicit. Context usage is shown after a chat response returns a `usage` payload, including input tokens, output tokens, and total token usage against the loaded `ctx-size`.
