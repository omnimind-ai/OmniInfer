...
- TUI `/status` shows the current backend, loaded model, context size, request defaults, and the most recent response usage. For llama.cpp-compatible backends, the TUI reads backend `/props` when the load-time `ctx-size` was not explicit. Context usage is shown after a chat response returns a `usage` payload; it is calculated from the last response's `total_tokens` against the loaded `ctx-size`.
