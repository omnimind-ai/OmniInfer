...
- The TUI keeps a fixed bottom input bar during chat so the conversation history stays visible above the prompt. The prompt area also shows a compact status line with the active backend, loaded model, thinking mode, recent context usage, and transient notices from backend/model/thinking commands.
