...
| `/omni/backends` | GET | 列出后端（scope: installed/compatible/all） |
| `/omni/backend/props` | GET | 当前后端属性 |
| `/omni/supported-models` | GET | 内置支持模型目录 |
| `/omni/supported-models/best` | GET | 按最佳已安装后端合并的支持模型目录 |
| `/omni/model/select` | POST | 加载模型 |
| `/omni/model/clear-selection` | POST | 禁用启动恢复，但不停止当前模型 |
| `/omni/backend/select` | POST | 选择后端 |
| `/omni/backend/stop` | POST | 临时停止当前后端，并保留恢复选择 |
| `/omni/cache/clear` | POST | 清除 KV 缓存（无需重新加载模型） |
| `/omni/thinking` | GET | 读取默认思考模式 |
| `/omni/thinking/select` | POST | 更新默认思考模式 |
...
`GET /omni/state` 通过 `restore_selection`、`restore_status` 和 `restore_completed` 区分当前运行时与启动恢复意图。非交互 direct `serve` 默认恢复持久选择；`--no-restore-model` 仅对本次启动禁用恢复。`POST /omni/backend/stop` 会保留该选择。如需阻止后续恢复，同时不停止当前已加载模型：
...
在不重新加载模型的情况下清除后端的 KV 缓存（适用于释放多轮对话累积占用的显存）：
