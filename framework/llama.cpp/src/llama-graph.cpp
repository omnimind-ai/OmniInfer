...
void llm_graph_input_lora_side::set_input(const llama_ubatch * ubatch) {
    if (adapter == nullptr || !adapter->zo_fused_htp || row_side == nullptr) {
        throw std::runtime_error("invalid HTP ZO-LoRA side input");
    }

    const int32_t side = llama_adapter_lora_get_zo_side(adapter);
    size_t split = 0;
    if (side < 0) {
        if (!ubatch->equal_seqs() || ubatch->n_seqs < 2 || ubatch->n_seqs % 2 != 0 ||
            ubatch->n_seq_tokens == 0 || ubatch->n_tokens != ubatch->n_seq_tokens*ubatch->n_seqs ||
            ubatch->n_seqs_unq != ubatch->n_seqs || ubatch->n_seq_id == nullptr || ubatch->seq_id == nullptr) {
            throw std::runtime_error("paired HTP ZO-LoRA requires one equal-sequence matrix");
        }

        const uint32_t n_side_seqs = ubatch->n_seqs/2;
        llama_seq_id previous_id = -1;
        for (uint32_t sequence = 0; sequence < ubatch->n_seqs; ++sequence) {
            const uint32_t begin = sequence*ubatch->n_seq_tokens;
            if (ubatch->n_seq_id[begin] != 1 || ubatch->seq_id[begin] == nullptr) {
                throw std::runtime_error("paired HTP ZO-LoRA requires one sequence id per token");
            }
            const llama_seq_id sequence_id = ubatch->seq_id[begin][0];
            if (sequence_id <= previous_id) {
                throw std::runtime_error("paired HTP ZO-LoRA sequence ids are not side-major");
            }
            previous_id = sequence_id;
            for (uint32_t token = 1; token < ubatch->n_seq_tokens; ++token) {
                const uint32_t index = begin + token;
                if (ubatch->n_seq_id[index] != 1 || ubatch->seq_id[index] == nullptr ||
                    ubatch->seq_id[index][0] != sequence_id) {
                    throw std::runtime_error("paired HTP ZO-LoRA sequence layout is not rectangular");
                }
            }
        }

        split = (size_t) n_side_seqs*ubatch->n_seq_tokens;
    }

    if (output_only) {
        if (ubatch->output == nullptr) {
            throw std::runtime_error("HTP ZO-LoRA output side input requires output flags");
        }

        values.clear();
        values.reserve(row_side->ne[0]);
        for (uint32_t token = 0; token < ubatch->n_tokens; ++token) {
            if (ubatch->output[token]) {
                values.push_back(side >= 0 ? side : (token < split ? 0 : 1));
            }
        }
        if (values.size() != (size_t) row_side->ne[0]) {
            throw std::runtime_error("HTP ZO-LoRA output side count does not match graph");
        }
    } else {
        if (row_side->ne[0] != ubatch->n_tokens) {
            throw std::runtime_error("HTP ZO-LoRA token side count does not match graph");
        }

        values.resize(ubatch->n_tokens);
        if (side >= 0) {
            std::fill(values.begin(), values.end(), side);
        } else {
            std::fill(values.begin(), values.begin() + split, 0);
            std::fill(values.begin() + split, values.end(), 1);
        }
    }

    ggml_backend_tensor_set(row_side, values.data(), 0, values.size()*sizeof(values[0]));
}

bool llm_graph_input_lora_side::can_reuse(const llm_graph_params & params) {
    const int64_t n_rows = output_only ? params.n_outputs : params.ubatch.n_tokens;
    return row_side != nullptr && row_side->ne[0] == n_rows &&
        params.loras != nullptr && params.loras->size() == 1 &&
        params.loras->begin()->first == adapter;
}
...
static bool llm_graph_backend_is_htp(ggml_backend_t backend) {
    if (backend == nullptr) {
        return false;
    }
    ...
    return name != nullptr && strcmp(name, "HTP") == 0;
}

static ggml_backend_t llm_graph_htp_backend_for_tensor(
    ...
    for (int index = 0; index < ggml_backend_sched_get_n_backends(sched); ++index) {
        ...
        if (ggml_backend_get_device(backend) == device && llm_graph_backend_is_htp(backend)) {
            return backend;
        }
    }
...
ggml_tensor * llm_graph_context::build_lora_mm(
    ...
    if (loras->size() == 1) {
        const auto & lora = *loras->begin();
        llama_adapter_lora * adapter = lora.first;
        llama_adapter_lora_weight * lw = adapter->get_weight(w);

        if (adapter->zo_fused_htp && lw != nullptr) {
            if (lw->b_pair == nullptr) {
                throw std::runtime_error("HTP ZO-LoRA B pair is not initialized for " + std::string(w->name));
            }
            if (cur->ne[2] != 1 || cur->ne[3] != 1) {
                throw std::runtime_error("HTP ZO-LoRA requires a plain 2D token batch");
            }

            const float scale = lw->get_scale(adapter->alpha, lora.second);
            ggml_tensor * base = ggml_mul_mat(ctx0, w, cur);
            ggml_tensor * tmp = ggml_mul_mat(ctx0, lw->a, cur);
            ggml_tensor * row_side = build_inp_lora_side(adapter, cur->ne[1]);
            ggml_tensor * res = ggml_lora_acc_inplace(
                    ctx0, base, tmp, lw->b_pair, row_side, scale,
                    GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK |
                    GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS);

            ggml_backend_t backend = llm_graph_htp_backend_for_tensor(sched, w);
            if (backend == nullptr ||
                llm_graph_htp_backend_for_tensor(sched, lw->a) != backend ||
                llm_graph_htp_backend_for_tensor(sched, lw->b_pair) != backend) {
                throw std::runtime_error("HTP ZO-LoRA tensors do not share one HTP backend for " + std::string(w->name));
            }
            ggml_backend_t side_backend = ggml_backend_sched_get_tensor_backend(sched, row_side);
            if (side_backend != nullptr && side_backend != backend) {
                throw std::runtime_error("HTP ZO-LoRA side input spans multiple HTP backends");
            }
            ggml_backend_sched_set_tensor_backend(sched, base, backend);
            ggml_backend_sched_set_tensor_backend(sched, tmp, backend);
            ggml_backend_sched_set_tensor_backend(sched, res, backend);
            ggml_backend_sched_set_tensor_backend(sched, row_side, backend);
        ...
        for (const auto & lora : *loras) {
            if (lora.first->zo_fused_htp) {
                throw std::runtime_error("HTP ZO-LoRA supports exactly one adapter");
            }
        }
    }
...
ggml_tensor * llm_graph_context::build_inp_lora_side(
        llama_adapter_lora * adapter,
        int64_t n_rows) const {
    bool output_only = false;
    if (n_rows == n_tokens) {
        output_only = false;
    } else if (n_rows == n_outputs) {
        output_only = true;
    } else {
        throw std::runtime_error("HTP ZO-LoRA token rows do not match graph inputs");
    }

    ggml_tensor *& row_side = output_only ? inp_lora_side_out : inp_lora_side;
    if (row_side != nullptr) {
        return row_side;
    }

    auto inp = std::make_unique<llm_graph_input_lora_side>(adapter, output_only);
    inp->row_side = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_rows);
    ggml_set_input(inp->row_side);
    cb(inp->row_side, output_only ? "inp_lora_side_out" : "inp_lora_side", -1);
    row_side = inp->row_side;
    res->add_input(std::move(inp));
    return row_side;
}
