...
static constexpr int64_t LLAMA_ADAPTER_LORA_HTP_Q4_K_ALIGNMENT = 256;
static constexpr int64_t LLAMA_ADAPTER_LORA_HTP_Q8_K_ALIGNMENT = 256;
...
static bool llama_adapter_lora_zo_target(const std::string & name) {
...
static bool llama_adapter_lora_is_htp_device(ggml_backend_dev_t dev) {
    if (dev == nullptr) {
        return false;
    }
...
static void llama_adapter_lora_init_impl(
    ...
    adapter.zo_fused_htp = zo_fused_htp;
    adapter.zo_paired    = zo_paired;
    adapter.zo_side      = zo_paired ? -1 : 0;

    const size_t ctx_tensor_slots = (size_t) n_tensors * (zo_fused_htp ? 2u : 1u);
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            // add a new context
            ggml_init_params params = {
                /*.mem_size   =*/ ctx_tensor_slots*ggml_tensor_overhead(),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
    ...
    for (ggml_tensor * cur = ggml_get_first_tensor(ctx.get()); cur; cur = ggml_get_next_tensor(ctx.get(), cur)) {
        ...
        } else if (str_endswith(name, "_norm.weight") && !zo_mode) {
    ...
    if (zo_mode && ab_map.empty()) {
        throw std::runtime_error("ZO-LoRA adapter contains no target tensors");
    }
    ...
    for (auto & it : ab_map) {
        ...
        if (zo_mode) {
            if (!llama_adapter_lora_zo_target(name)) {
                throw std::runtime_error("unsupported ZO-LoRA target tensor '" + name + "'");
            }
            if (w.a->type != GGML_TYPE_F16 || w.b->type != GGML_TYPE_F16) {
                throw std::runtime_error("ZO-LoRA requires F16 A/B tensors for '" + name + "'");
            }
            if (w.a->ne[2] != 1 || w.a->ne[3] != 1 || w.b->ne[2] != 1 || w.b->ne[3] != 1) {
                throw std::runtime_error("ZO-LoRA requires 2D A/B tensors for '" + name + "'");
            }
            const int64_t rank = w.a->ne[1];
            if (rank != 8 && rank != 16 && rank != 24 && rank != 32) {
                throw std::runtime_error("ZO-LoRA rank must be one of 8, 16, 24, 32");
            }
            if (zo_rank == -1) {
                zo_rank = rank;
            } else if (zo_rank != rank) {
                throw std::runtime_error("ZO-LoRA requires a uniform rank across all tensors");
            }
        }
        ...
        if (zo_mode && (model_tensor->ne[2] != 1 || model_tensor->ne[3] != 1)) {
            throw std::runtime_error("ZO-LoRA requires a 2D base tensor for '" + name + "'");
        }
        ...
        if (zo_fused_htp) {
            ...
            if (!llama_adapter_lora_is_htp_device(dev)) {
                throw std::runtime_error("ZO-LoRA target tensor '" + name + "' is not placed on HTP");
            }
            if (model_tensor->type != GGML_TYPE_Q4_0 &&
                model_tensor->type != GGML_TYPE_Q8_0 &&
                model_tensor->type != GGML_TYPE_F16) {
                throw std::runtime_error("HTP ZO-LoRA requires Q4_0, Q8_0, or F16 base tensor '" + name + "'");
            }
            if ((model_tensor->type == GGML_TYPE_Q4_0 &&
                 model_tensor->ne[0] % LLAMA_ADAPTER_LORA_HTP_Q4_K_ALIGNMENT != 0) ||
                (model_tensor->type == GGML_TYPE_Q8_0 &&
                 model_tensor->ne[0] % LLAMA_ADAPTER_LORA_HTP_Q8_K_ALIGNMENT != 0) ||
                model_tensor->ne[1] % 32 != 0) {
                throw std::runtime_error("HTP ZO-LoRA tensor '" + name + "' does not satisfy fused matmul alignment");
            }
        ...
        for (auto & ex : buft_extra) {
            if (ex == buft) {
                if (zo_fused_htp) {
                    throw std::runtime_error("HTP ZO-LoRA workspace for '" + name + "' cannot use the target buffer type");
                }
        ...
        ggml_tensor * tensor_b_pair = zo_fused_htp ? ggml_new_tensor_4d(
            dev_ctx, GGML_TYPE_F16, ((w.b->ne[1] + 63)/64)*64, w.b->ne[0], 2, 1) : nullptr;
        ggml_set_name(tensor_a, w.a->name);
        ggml_set_name(tensor_b, w.b->name);
        if (tensor_b_pair != nullptr) {
            ggml_set_name(tensor_b_pair, (name + ".lora_b_pair").c_str());
        }
        adapter.ab_map[name] = llama_adapter_lora_weight(tensor_a, tensor_b, tensor_b_pair);
    }
    ...
    // set tensor data
    {
        ...
        for (auto & it : adapter.ab_map) {
            ...
            if (dev.b_pair != nullptr) {
                read_buf.assign(ggml_nbytes(dev.b_pair), 0);
                ggml_backend_tensor_set(dev.b_pair, read_buf.data(), 0, read_buf.size());
            }
        }
    }
...
llama_adapter_lora * llama_adapter_lora_init(llama_model * model, const char * path_lora) {
    ...
    try {
        llama_adapter_lora_init_impl(*model, path_lora, *adapter, false, false, false);
        return adapter;
...
llama_adapter_lora * llama_adapter_lora_init_zo(
    ...
    try {
        llama_adapter_lora_init_impl(*model, path_lora, *adapter, true, fused_htp, paired);
        return adapter;
    } catch (const std::exception & err) {
        LLAMA_LOG_ERROR("%s: failed to load ZO-LoRA adapter: %s\n", __func__, err.what());
        delete adapter;
    }
...
void llama_adapter_lora_set_zo_side(llama_adapter_lora * adapter, int32_t side) {
    if (adapter == nullptr || side < -1 || side > 1 || (side == -1 && !adapter->zo_paired)) {
        throw std::invalid_argument("invalid ZO-LoRA side");
    }
    adapter->zo_side = side;
}

int32_t llama_adapter_lora_get_zo_side(const llama_adapter_lora * adapter) {
    if (adapter == nullptr) {
        throw std::invalid_argument("null ZO-LoRA adapter");
    }
    return adapter->zo_side;
}
