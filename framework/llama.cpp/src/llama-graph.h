...
class llm_graph_input_lora_side : public llm_graph_input_i {
public:
    llm_graph_input_lora_side(llama_adapter_lora * adapter, bool output_only) :
        adapter(adapter), output_only(output_only) {}
    virtual ~llm_graph_input_lora_side() = default;

    void set_input(const llama_ubatch * ubatch) override;

    bool can_reuse(const llm_graph_params & params) override;

    ggml_tensor * row_side = nullptr; // I32 [n_rows]
...
struct llm_graph_context {
    ...
    mutable ggml_tensor * inp_lora_side     = nullptr;
    mutable ggml_tensor * inp_lora_side_out = nullptr;
    ...
    ggml_tensor * build_inp_lora_side(llama_adapter_lora * adapter, int64_t n_rows) const;
