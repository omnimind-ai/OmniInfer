...
static bool llama_backend_is_htp(ggml_backend_t backend) {
    if (backend == nullptr) {
        return false;
    }
...
static bool llama_audit_zo_htp_graph(
        ggml_backend_sched_t sched,
        ggml_cgraph * gf,
        llama_adapter_lora * adapter,
        bool require_flash_attn) {
    size_t n_acc = 0;
    size_t n_flash_attn = 0;
    std::unordered_set<const llama_adapter_lora_weight *> seen_weights;

    for (int index = 0; index < ggml_graph_n_nodes(gf); ++index) {
        ggml_tensor * node = ggml_graph_node(gf, index);
        if (node->op == GGML_OP_LORA_ACC_INPLACE) {
            ++n_acc;
            ggml_tensor * base = node->src[0];
            ggml_tensor * tmp = node->src[1];
            if (base == nullptr || tmp == nullptr || base->op != GGML_OP_MUL_MAT || tmp->op != GGML_OP_MUL_MAT ||
                base->src[0] == nullptr || tmp->src[0] == nullptr || node->src[2] == nullptr || node->src[3] == nullptr) {
                LLAMA_LOG_ERROR("%s: malformed HTP ZO-LoRA graph at node %s\n", __func__, node->name);
                return false;
            }

            const auto * weight = adapter->get_weight(base->src[0]);
            if (weight == nullptr || weight->a != tmp->src[0] || weight->b_pair != node->src[2] ||
                !seen_weights.insert(weight).second) {
                LLAMA_LOG_ERROR("%s: HTP ZO-LoRA target mismatch at node %s\n", __func__, node->name);
                return false;
            }

            ggml_backend_t backend = ggml_backend_sched_get_tensor_backend(sched, node);
            if (!llama_backend_is_htp(backend) ||
                ggml_backend_sched_get_tensor_backend(sched, base) != backend ||
                ggml_backend_sched_get_tensor_backend(sched, tmp) != backend ||
                ggml_backend_sched_get_tensor_backend(sched, node->src[2]) != backend ||
                ggml_backend_sched_get_tensor_backend(sched, node->src[3]) != backend) {
                LLAMA_LOG_ERROR("%s: HTP ZO-LoRA node escaped its target backend: %s\n", __func__, node->name);
                return false;
            }
        } else if (node->op == GGML_OP_FLASH_ATTN_EXT) {
            ++n_flash_attn;
            if (!llama_backend_is_htp(ggml_backend_sched_get_tensor_backend(sched, node))) {
                LLAMA_LOG_ERROR("%s: Flash Attention node is not allocated on HTP: %s\n", __func__, node->name);
                return false;
            }
        }
    }

    if (n_acc != adapter->ab_map.size() || seen_weights.size() != adapter->ab_map.size()) {
        LLAMA_LOG_ERROR("%s: expected %zu HTP ZO-LoRA targets, audited %zu\n",
            __func__, adapter->ab_map.size(), n_acc);
        return false;
    }
    if (require_flash_attn && n_flash_attn == 0) {
        LLAMA_LOG_ERROR("%s: fused HTP ZO-LoRA requires an HTP Flash Attention graph\n", __func__);
        return false;
    }
    ...
    if (!graph_reuse_disable && res->can_reuse(gparams)) {
        ...
        for (const auto & lora : *loras) {
            if (lora.first->zo_fused_htp &&
                !llama_audit_zo_htp_graph(sched.get(), gf, lora.first, cparams.flash_attn)) {
                ret = GGML_STATUS_FAILED;
                return nullptr;
            }
        }
