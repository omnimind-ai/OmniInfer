...
extern "C" {
    ...
    enum ggml_lora_acc_flags {
        GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK = 1 << 0,
        GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS  = 1 << 1,
    };

    struct ggml_lora_acc_params {
        float   scale;
        int32_t flags;
    };
    ...
    // base += scale*b_pair[row_side]*(tmp)
    // b_pair has shape [round_up(output_dim, 64), rank, 2, 1]
    GGML_API struct ggml_tensor * ggml_lora_acc_inplace(
            struct ggml_context * ctx,
