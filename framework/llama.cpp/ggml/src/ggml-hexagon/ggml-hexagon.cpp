...
#            define GGML_HEXAGON_HAVE_LINUX_DMA_BUF_H 1
#        endif
#    endif
#    ifndef DMA_BUF_SYNC_WRITE
#        define DMA_BUF_SYNC_WRITE (2ull << 0)
#    endif
#    ifndef DMA_BUF_SYNC_START
#        define DMA_BUF_SYNC_START (0ull << 2)
#    endif
#    ifndef DMA_BUF_SYNC_END
...
#        ifndef GGML_HEXAGON_HAVE_LINUX_DMA_BUF_H
struct dma_buf_sync {
    uint64_t flags;
};
#        endif
...
#    endif
#    define GGML_HEXAGON_HAS_DMA_BUF_SYNC 1
#endif
...
static int opt_opbatch  = HTP_OP_MAX_REQS; // max number of ops in a batch
...
struct ggml_hexagon_shared_buffer {
    ...
    bool                   mapped;
    bool                   pinned;
    bool                   host_write_active;
    uint64_t               write_generation;
    std::unordered_map<uint64_t, tensor_generation> tensor_generations;
    ...
    ggml_hexagon_shared_buffer(ggml_hexagon_session * sess, size_t size, bool pinned = false) {
        ...
        this->mapped = false;
        this->pinned = pinned;
        this->host_write_active = false;
        this->write_generation  = 0;
...
static bool ggml_hexagon_is_dynamic_lora_b_pair(const ggml_tensor * tensor) {
    static const char suffix[] = ".lora_b_pair";
    if (tensor == nullptr) {
        return false;
    }
    ...
    return name_len >= suffix_len && memcmp(tensor->name + name_len - suffix_len, suffix, suffix_len) == 0;
}

static bool ggml_hexagon_host_write_sync(
...
#if !defined(_WIN32) && defined(GGML_HEXAGON_HAS_DMA_BUF_SYNC)
    struct dma_buf_sync sync = {};
    sync.flags = (start ? DMA_BUF_SYNC_START : DMA_BUF_SYNC_END) | DMA_BUF_SYNC_WRITE;
    if (ioctl(sbuf->fd, DMA_BUF_IOCTL_SYNC, &sync) == 0) {
        return true;
    }

    static std::atomic<bool> dma_sync_warned{false};
    if (!dma_sync_warned.exchange(true)) {
        GGML_LOG_WARN("ggml-hex: DMA_BUF_IOCTL_SYNC failed for dynamic LoRA data (%s)\n", strerror(errno));
    }
#else
    GGML_UNUSED(sbuf);
#endif
    ...
    if (!start) {
        ...
        const uintptr_t begin = (uintptr_t) data & ~page_mask;
        const uintptr_t end = ((uintptr_t) data + size + page_mask) & ~page_mask;
        if (msync((void *) begin, end - begin, MS_SYNC) != 0) {
            return false;
        }
    }
#else
...
static void ggml_hexagon_set_tensor_raw(
        ggml_hexagon_shared_buffer * sbuf,
        ggml_tensor *                tensor,
        const void *                 data,
        size_t                       offset,
    ...
    const bool dynamic_lora_b_pair = ggml_hexagon_is_dynamic_lora_b_pair(tensor);
    const bool standalone_write = dynamic_lora_b_pair && !sbuf->host_write_active;

    if (standalone_write) {
        sbuf->sess->flush();
        if (!ggml_hexagon_host_write_sync(sbuf, dst, size, true)) {
            throw std::runtime_error("ggml-hex: failed to begin dynamic LoRA cache synchronization");
        }
    }

    memcpy(dst, data, size);

    if (dynamic_lora_b_pair) {
        if (standalone_write && !ggml_hexagon_host_write_sync(sbuf, dst, size, false)) {
            throw std::runtime_error("ggml-hex: failed to publish dynamic LoRA data to the DSP");
        }
        const uint64_t tensor_offset = (uint8_t *) tensor->data - sbuf->base;
        auto & generation = sbuf->tensor_generations[tensor_offset];
        generation.written = ++sbuf->write_generation;
    }
}

static bool ggml_backend_hexagon_buffer_host_write_begin(ggml_backend_buffer_t buffer) {
    auto * sbuf = static_cast<ggml_hexagon_shared_buffer *>(buffer->context);
    if (sbuf->host_write_active) {
        GGML_LOG_ERROR("ggml-hex: %s host-write transaction is already active\n", sbuf->sess->c_name());
        return false;
    }

    sbuf->sess->flush();
    if (!ggml_hexagon_host_write_sync(sbuf, sbuf->base, sbuf->size, true)) {
        return false;
    }

    sbuf->host_write_active = true;
    return true;
}

static bool ggml_backend_hexagon_buffer_host_write_end(ggml_backend_buffer_t buffer) {
    auto * sbuf = static_cast<ggml_hexagon_shared_buffer *>(buffer->context);
    if (!sbuf->host_write_active) {
        GGML_LOG_ERROR("ggml-hex: %s host-write transaction is not active\n", sbuf->sess->c_name());
        return false;
    }

    const bool success = ggml_hexagon_host_write_sync(sbuf, sbuf->base, sbuf->size, false);
    sbuf->host_write_active = false;
    return success;
}
...
static void ggml_backend_hexagon_buffer_set_tensor(ggml_backend_buffer_t buffer,
    ...
    switch (tensor->type) {
        ...
        default:
            ggml_hexagon_set_tensor_raw(sbuf, tensor, data, offset, size);
            break;
    }
}
...
static bool ggml_hexagon_encode_tensor_layout(
        const ggml_tensor * t,
        uint32_t            ne[HTP_OP_MAX_DIMS],
        uint32_t            nb[HTP_OP_MAX_DIMS],
        uint32_t *          size) {
    for (int dim = 0; dim < HTP_OP_MAX_DIMS; ++dim) {
        if (t->ne[dim] <= 0 || (uint64_t) t->ne[dim] > UINT32_MAX || t->nb[dim] > UINT32_MAX) {
            return false;
        }

        ne[dim] = (uint32_t) t->ne[dim];
        nb[dim] = (uint32_t) t->nb[dim];
    }

    const int64_t block_size = ggml_blck_size(t->type);
    if (block_size <= 0) {
        return false;
    }

    uint64_t span;
    int first_stride_dim;
    if (block_size == 1) {
        span = ggml_type_size(t->type);
        first_stride_dim = 0;
    } else {
        if ((uint64_t) ne[0] % (uint64_t) block_size != 0) {
            return false;
        }
        if (nb[0] != 0 && ne[0] > UINT64_MAX / nb[0]) {
            return false;
        }
        span = (uint64_t) ne[0] * nb[0] / (uint64_t) block_size;
        first_stride_dim = 1;
    }
    ...
    for (int dim = first_stride_dim; dim < HTP_OP_MAX_DIMS; ++dim) {
        const uint64_t count = (uint64_t) ne[dim] - 1;
        if (count != 0 && nb[dim] > (UINT32_MAX - span) / count) {
            return false;
        }
        span += count * nb[dim];
    }
...
struct ggml_hexagon_opbatch {
    struct dirty_generation {
        ggml_hexagon_shared_buffer * buffer;
        uint64_t offset;
        uint64_t generation;
    };
    ...
    std::vector<dirty_generation>               dirty_generations;
    ...
    int add_tensor(const ggml_tensor * t) {
        ...
        uint32_t t_ne[HTP_OP_MAX_DIMS];
        uint32_t t_nb[HTP_OP_MAX_DIMS];
        uint32_t t_size;
        if (!ggml_hexagon_encode_tensor_layout(t, t_ne, t_nb, &t_size)) {
            GGML_ABORT("ggml-hex: tensor %s cannot be represented by an HTP descriptor\n", t->name);
        }

        const uintptr_t buffer_addr = (uintptr_t) sbuf->base;
        const uintptr_t tensor_addr = (uintptr_t) t->data;
        if (tensor_addr < buffer_addr) {
            GGML_ABORT("ggml-hex: tensor %s lies outside its HTP buffer\n", t->name);
        }

        const uint64_t t_offset_u64 = tensor_addr - buffer_addr;
        if (t_offset_u64 > UINT32_MAX || t_offset_u64 > sbuf->size ||
            t_size > sbuf->size - (size_t) t_offset_u64) {
            GGML_ABORT("ggml-hex: tensor %s range cannot be represented by an HTP descriptor\n", t->name);
        }
        const uint32_t t_offset = (uint32_t) t_offset_u64;
        ...
        h.size  = t_size;
        h.type  = t->type;
        memcpy(h.ne, t_ne, sizeof(t_ne));
        memcpy(h.nb, t_nb, sizeof(t_nb));

        h.flags = 0;
        if (ggml_backend_buffer_get_usage(t->buffer) == GGML_BACKEND_BUFFER_USAGE_COMPUTE) {
            h.flags |= HTP_TENSOR_COMPUTE;
        }
        const auto generation_it = sbuf->tensor_generations.find(t_offset);
        if (generation_it != sbuf->tensor_generations.end() &&
            generation_it->second.written > generation_it->second.submitted) {
            h.flags |= HTP_TENSOR_HOST_DIRTY;
            dirty_generations.push_back({ sbuf, t_offset, generation_it->second.written });
        }

        HEX_VERBOSE("ggml-hex: add-tensor #%u %s : bi %d data %p offset %zu size %zu flags 0x%x : %zu:%zu:%zu:%zu\n",
                ti, t->name, h.bi, (void*) t->data, (size_t) t_offset, (size_t) t_size, h.flags,
    ...
    void mark_submitted() {
        for (const auto & dirty : dirty_generations) {
            const auto it = dirty.buffer->tensor_generations.find(dirty.offset);
            if (it != dirty.buffer->tensor_generations.end()) {
                it->second.submitted = std::max(it->second.submitted, dirty.generation);
            }
        }
    }
...
void ggml_hexagon_session::flush_pending(bool all) {
    while (this->op_pending) {
        ...
        if (rsp.status != HTP_STATUS_OK) {
            ...
            GGML_ABORT("ggml-hex: DSP operation failed with status %u\n", rsp.status);
        }
...
static bool ggml_hexagon_fa_descriptor_valid(const struct ggml_tensor * tensor) {
    if (tensor == nullptr) {
        return false;
    }

    uint64_t span = ggml_type_size(tensor->type);
    for (int dim = 0; dim < GGML_MAX_DIMS; ++dim) {
        if (tensor->ne[dim] <= 0 || (uint64_t) tensor->ne[dim] > UINT32_MAX ||
            tensor->nb[dim] == 0 || tensor->nb[dim] > UINT32_MAX) {
            return false;
        }

        const uint64_t count = (uint64_t) tensor->ne[dim] - 1;
        if (tensor->nb[dim] != 0 && count > (UINT32_MAX - span) / tensor->nb[dim]) {
            return false;
        }
        span += count * tensor->nb[dim];
    }

    return span <= UINT32_MAX;
}
...
static bool ggml_hexagon_supported_flash_attn_ext(const struct ggml_hexagon_session * sess, const struct ggml_tensor * op) {
    ...
    if (q == nullptr || k == nullptr || v == nullptr ||
        !ggml_hexagon_fa_descriptor_valid(q) || !ggml_hexagon_fa_descriptor_valid(k) ||
        !ggml_hexagon_fa_descriptor_valid(v) || !ggml_hexagon_fa_descriptor_valid(dst)) {
        return false;
    }
    ...
    if (q->nb[0] != ggml_type_size(q->type) || k->nb[0] != sizeof(ggml_fp16_t) ||
        v->nb[0] != sizeof(ggml_fp16_t) || dst->nb[0] != ggml_type_size(dst->type)) {
        return false;
    }

    if (q->ne[0] != k->ne[0] || k->ne[1] != v->ne[1] || k->ne[2] != v->ne[2] ||
        q->ne[2] > 512 || q->ne[2] % k->ne[2] != 0 ||
        q->ne[3] % k->ne[3] != 0 || q->ne[3] % v->ne[3] != 0 ||
        dst->ne[0] != v->ne[0] || dst->ne[1] != q->ne[2] ||
        dst->ne[2] != q->ne[1] || dst->ne[3] != q->ne[3]) {
        return false;
    }

    if (mask != nullptr &&
        (!ggml_hexagon_fa_descriptor_valid(mask) || mask->type != GGML_TYPE_F16 ||
         !ggml_is_contiguous(mask) || mask->ne[0] < k->ne[1] || mask->ne[1] < q->ne[1] ||
         q->ne[2] % mask->ne[2] != 0 || q->ne[3] % mask->ne[3] != 0)) {
        return false;
    }

    if (sinks != nullptr &&
        (!ggml_hexagon_fa_descriptor_valid(sinks) || sinks->type != GGML_TYPE_F32 ||
         !ggml_is_contiguous(sinks) || sinks->ne[0] != q->ne[2] ||
         sinks->ne[1] != 1 || sinks->ne[2] != 1 || sinks->ne[3] != 1)) {
        return false;
    }
    ...
    memcpy(&scale,         (const float *) op->op_params + 0, sizeof(scale));
    ...
    const enum ggml_prec precision = ggml_flash_attn_ext_get_prec(op);
    if ((precision != GGML_PREC_DEFAULT && precision != GGML_PREC_F32) ||
        !std::isfinite(scale) || !std::isfinite(max_bias) || max_bias < 0.0f ||
...
static bool ggml_hexagon_supported_lora_acc_inplace(
        const struct ggml_hexagon_session * sess,
        const struct ggml_tensor *           op) {
    const struct ggml_tensor * base     = op->src[0];
    const struct ggml_tensor * tmp      = op->src[1];
    const struct ggml_tensor * b_pair   = op->src[2];
    const struct ggml_tensor * row_side = op->src[3];

    if (!base || !tmp || !b_pair || !row_side) {
        return false;
    }

    struct ggml_lora_acc_params params = {};
    memcpy(&params, op->op_params, sizeof(params));

    const int64_t rank = tmp->ne[0];
    if ((rank != 8 && rank != 16 && rank != 24 && rank != 32) ||
        !std::isfinite(params.scale) ||
        !(params.flags & GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK) ||
        !(params.flags & GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS) ||
        (params.flags & ~(GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK |
                          GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS)) != 0 ||
        base->type != GGML_TYPE_F32 || tmp->type != GGML_TYPE_F32 ||
        b_pair->type != GGML_TYPE_F16 || row_side->type != GGML_TYPE_I32 ||
        op->type != GGML_TYPE_F32) {
        return false;
    }

    if (base->ne[2] != 1 || base->ne[3] != 1 || tmp->ne[2] != 1 || tmp->ne[3] != 1 ||
        b_pair->ne[2] != 2 || b_pair->ne[3] != 1 ||
        ggml_nelements(row_side) != base->ne[1] || row_side->ne[0] != base->ne[1] ||
        tmp->ne[1] != base->ne[1] || b_pair->ne[1] != rank ||
        b_pair->ne[0] != ((base->ne[0] + 63)/64)*64 ||
        !ggml_are_same_shape(base, op) || base->ne[0] % 32 != 0) {
        return false;
    }

    if (!ggml_is_contiguous(base) || !ggml_is_contiguous(tmp) ||
        !ggml_is_contiguous(b_pair) || !ggml_is_contiguous(row_side) ||
...
static htp_op_code op_remap_to_htp(const ggml_tensor * t) {
    switch (t->op) {
        ...
        case GGML_OP_LORA_ACC_INPLACE:return HTP_OP_LORA_ACC_INPLACE;
...
static bool ggml_backend_hexagon_device_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_LORA_ACC_INPLACE:
            supp = ggml_hexagon_supported_lora_acc_inplace(sess, op);
            break;
...
static void * ggml_backend_hexagon_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    ...
    if (strcmp(name, "ggml_backend_buffer_host_write_begin") == 0) {
        ggml_backend_buffer_host_write_t fct = ggml_backend_hexagon_buffer_host_write_begin;
        return (void *) fct;
    }

    if (strcmp(name, "ggml_backend_buffer_host_write_end") == 0) {
        ggml_backend_buffer_host_write_t fct = ggml_backend_hexagon_buffer_host_write_end;
        return (void *) fct;
    }
...
static bool ggml_hexagon_init(ggml_backend_reg * reg) {
    ...
    int detected_arch = 0;
    int err = get_hex_arch_ver(CDSP_DOMAIN_ID, &detected_arch);
    if (err != 0) {
        GGML_LOG_ERROR("ggml-hex: failed to query HTP version (err %d)\n", err);
        return false;
    }

    opt_arch = detected_arch;
    if (str_arch) {
        if (str_arch[0] == 'v' || str_arch[0] == 'V') {
            str_arch++;
        }
        char * end = nullptr;
        const int requested_arch = strtoul(str_arch, &end, 0);
        if (end == str_arch || *end != '\0') {
            GGML_LOG_ERROR("ggml-hex: invalid Hexagon architecture override: %s\n", str_arch);
            return false;
        }
        if (requested_arch != detected_arch) {
            GGML_LOG_ERROR("ggml-hex: requested v%d but detected v%d\n", requested_arch, detected_arch);
            return false;
        }
    }

    if (opt_arch != 68 && opt_arch != 69 && opt_arch != 73 &&
        opt_arch != 75 && opt_arch != 79 && opt_arch != 81) {
        GGML_LOG_ERROR("ggml-hex: unsupported Hexagon architecture v%d\n", opt_arch);
        return false;
    }
    ...
    if (opt_ndev == 0) {
        GGML_LOG_ERROR("ggml-hex: GGML_HEXAGON_NDEV must be at least 1\n");
        return false;
    }
    if (opt_opbatch > HTP_OP_MAX_REQS) {
        GGML_LOG_WARN("ggml-hex: limiting GGML_HEXAGON_OPBATCH to %u\n", (unsigned) HTP_OP_MAX_REQS);
        opt_opbatch = HTP_OP_MAX_REQS;
    }
    if (opt_opbatch <= 0) {
        GGML_LOG_ERROR("ggml-hex: GGML_HEXAGON_OPBATCH must be at least 1\n");
        return false;
    }
    ...
    if (registry->devices[0].context == nullptr) {
        delete registry;
        return false;
    }
