...
#define FLASH_ATTN_HMX_MIN_TOKENS 32

#define HTP_PREC_DEFAULT 0
#define HTP_PREC_F32    10
...
static bool htp_fa_tensor_span_valid(const struct htp_tensor * tensor, uint32_t element_size) {
    if (tensor == NULL || tensor->data == 0 || tensor->size == 0) {
        return false;
    }

    uint64_t span = element_size;
    for (int dim = 0; dim < HTP_OP_MAX_DIMS; ++dim) {
        ...
        if (count != 0 && tensor->nb[dim] > (UINT32_MAX - span) / count) {
            return false;
        }
        span += count * tensor->nb[dim];
    }

    return span <= tensor->size;
}

static bool htp_fa_tensor_contiguous(const struct htp_tensor * tensor, uint32_t element_size) {
    uint64_t stride = element_size;
    for (int dim = 0; dim < HTP_OP_MAX_DIMS; ++dim) {
        if (tensor->nb[dim] != stride || tensor->ne[dim] > UINT32_MAX / stride) {
            return false;
        }
        stride *= tensor->ne[dim];
    }
    return true;
}

static int htp_fa_validate(const struct htp_ops_context * octx) {
    ...
    if (q == NULL || k == NULL || v == NULL || dst == NULL || octx->ctx == NULL || octx->n_threads == 0) {
        return HTP_STATUS_INVAL_PARAMS;
    }

    if ((q->type != HTP_TYPE_F16 && q->type != HTP_TYPE_F32) ||
        k->type != HTP_TYPE_F16 || v->type != HTP_TYPE_F16 ||
        (dst->type != HTP_TYPE_F16 && dst->type != HTP_TYPE_F32)) {
        return HTP_STATUS_NO_SUPPORT;
    }

    const uint32_t q_size   = q->type == HTP_TYPE_F32 ? sizeof(float) : sizeof(__fp16);
    const uint32_t dst_size = dst->type == HTP_TYPE_F32 ? sizeof(float) : sizeof(__fp16);
    if (!htp_fa_tensor_span_valid(q, q_size) || !htp_fa_tensor_span_valid(k, sizeof(__fp16)) ||
        !htp_fa_tensor_span_valid(v, sizeof(__fp16)) || !htp_fa_tensor_span_valid(dst, dst_size) ||
        q->nb[0] != q_size || k->nb[0] != sizeof(__fp16) ||
        v->nb[0] != sizeof(__fp16) || dst->nb[0] != dst_size) {
        return HTP_STATUS_INVAL_PARAMS;
    }
    ...
    if (mask != NULL) {
        if (mask->type != HTP_TYPE_F16 || !htp_fa_tensor_span_valid(mask, sizeof(__fp16)) ||
            !htp_fa_tensor_contiguous(mask, sizeof(__fp16)) ||
    ...
    if (sinks != NULL) {
        if (sinks->type != HTP_TYPE_F32 || !htp_fa_tensor_span_valid(sinks, sizeof(float)) ||
            !htp_fa_tensor_contiguous(sinks, sizeof(float)) || sinks->ne[0] != q->ne[2] ||
    ...
    int32_t precision   = 0;
    memcpy(&scale,         (const float *) octx->op_params + 0, sizeof(scale));
    memcpy(&max_bias,      (const float *) octx->op_params + 1, sizeof(max_bias));
    memcpy(&logit_softcap, (const float *) octx->op_params + 2, sizeof(logit_softcap));
    memcpy(&precision,     (const int32_t *) octx->op_params + 3, sizeof(precision));

    if ((precision != HTP_PREC_DEFAULT && precision != HTP_PREC_F32) ||
        !isfinite(scale) || !isfinite(max_bias) || max_bias < 0.0f ||
        !isfinite(logit_softcap) || logit_softcap < 0.0f ||
        (max_bias > 0.0f && mask == NULL)) {
        return HTP_STATUS_INVAL_PARAMS;
    }
...
static bool htp_fa_hmx_tensor_aligned(const struct htp_tensor * tensor) {
    if (((uintptr_t) tensor->data & 127) != 0) {
        return false;
    }
    for (int dim = 1; dim < HTP_OP_MAX_DIMS; ++dim) {
        if ((tensor->nb[dim] & 127) != 0) {
            return false;
        }
    }
    return true;
...
static bool htp_fa_hmx_eligible(const struct htp_ops_context * octx) {
    const struct htp_tensor * q    = octx->src[0];
    const struct htp_tensor * k    = octx->src[1];
    const struct htp_tensor * v    = octx->src[2];
    const struct htp_tensor * mask = octx->src[3];

    if (!octx->ctx->hmx_enabled || octx->src[4] != NULL || q->ne[1] < FLASH_ATTN_HMX_MIN_TOKENS ||
        q->ne[0] % 64 != 0 || v->ne[0] % 64 != 0 || (mask != NULL && mask->ne[2] != 1) ||
        !htp_fa_hmx_tensor_aligned(q) || !htp_fa_hmx_tensor_aligned(k) ||
        !htp_fa_hmx_tensor_aligned(v) || !htp_fa_hmx_tensor_aligned(octx->dst)) {
        return false;
    }

    return mask == NULL || htp_fa_hmx_tensor_aligned(mask);
}
...
static void flash_attn_ext_f16_thread(unsigned int nth, unsigned int ith, void * data) {
    ...
    for (uint32_t ir = ir0; ir < ir1; ++ir) {
        ...
        for (uint32_t ib = 0; ib < factx->n_blocks; ++ib) {
            ...
            const HVX_Vector mask_block = mask ? *(const HVX_UVector *) m_base : Q6_V_vzero();
            ...
            for (uint32_t iv = 0; ic < current_block_size; ic += VLEN_FP32, ++iv) {
                ...
                // 3. Mask
                if (mask) {
                    HVX_Vector m_vals_f16 = ic == 0 ? mask_block : Q6_V_vror_VR(mask_block, 64);
                    ...
                    HVX_Vector inf_lanes = Q6_V_vand_QV(is_inf, hvx_vec_splat_f16(1.0f));
                    HVX_VectorPair inf_f32 = hvx_vec_f16_to_f32(inf_lanes);
                    HVX_VectorPred is_inf_f32 = Q6_Q_vcmp_gt_VsfVsf(Q6_V_lo_W(inf_f32), Q6_V_vzero());
                    ...
                    #endif
                    scores = Q6_V_vmux_QVV(is_inf_f32, hvx_vec_splat_f32(-INFINITY), scores);
                }
...
int op_flash_attn_ext(struct htp_ops_context * octx) {
    ...
    const int validation_status = htp_fa_validate(octx);
    if (!htp_status_is_ok(validation_status)) {
        return validation_status;
    }

#ifdef HTP_HAS_HMX
    if (htp_fa_hmx_eligible(octx)) {
        int ret = hmx_flash_attn_ext(octx);
        if (htp_status_is_ok(ret)) {
            static bool reported_hmx_flash_attn_active = false;
            if (!reported_hmx_flash_attn_active) {
                FARF(ALWAYS, "HMX Flash Attention active");
                reported_hmx_flash_attn_active = true;
            }
            return HTP_STATUS_OK;
        }
        if (ret != HTP_STATUS_NO_SUPPORT && ret != HTP_STATUS_VTCM_TOO_SMALL) {
            return ret;
        }
        static bool reported_hmx_flash_attn_fallback = false;
        if (!reported_hmx_flash_attn_fallback) {
            FARF(ALWAYS, "HMX Flash Attention unavailable (status=%d); using HVX", ret);
            reported_hmx_flash_attn_fallback = true;
        }
    }
#endif
