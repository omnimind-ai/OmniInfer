...
static void fa_softmax_thread(unsigned int n, unsigned int i, void * data) {
    ...
    const HVX_Vector v_neg_inf = Q6_Vh_vsplat_R(0xfc00);
        ...
        for (int r_vec_off = 0; r_vec_off < 64; r_vec_off += 2) {
            ...
            for (size_t c = 0; c < kv_rows; c += 64) {
                ...
                if (args->mask) {
                    ...
                    HVX_VectorPred q_neg_inf0 = Q6_Q_vcmp_eq_VhVh(v_mask0, v_neg_inf);
                    HVX_VectorPred q_neg_inf1 = Q6_Q_vcmp_eq_VhVh(v_mask1, v_neg_inf);
                    HVX_VectorPred q_keep0 = Q6_Q_and_QQn(q_tail_keep, q_neg_inf0);
                    HVX_VectorPred q_keep1 = r + 1 < (int) n_rows_g ?
                        Q6_Q_and_QQn(q_tail_keep, q_neg_inf1) : Q6_Q_vsetq_R(0);
                    const HVX_Vector v_zero = Q6_V_vzero();
                    v_mask0 = Q6_V_vmux_QVV(q_keep0, v_mask0, v_zero);
                    v_mask1 = Q6_V_vmux_QVV(q_keep1, v_mask1, v_zero);
                    ...
                    if (args->has_alibi) {
                        ...
                        // No-ALiBi fast path (scheme A): slope is 1.0, skip the mul
                        // but still add the finite mask bias.
                ...
                } else if (ne < 64 || r + 1 >= (int) n_rows_g) {
                    HVX_VectorPred q_tail_keep1 = r + 1 < (int) n_rows_g ? q_tail_keep : Q6_Q_vsetq_R(0);
                    my_row_buf0[ci] = Q6_V_vmux_QVV(q_tail_keep,  my_row_buf0[ci], v_neg_inf);
                    my_row_buf1[ci] = Q6_V_vmux_QVV(q_tail_keep1, my_row_buf1[ci], v_neg_inf);
                }
...
#endif
                HVX_VectorPred q_neg_inf0 = Q6_Q_vcmp_eq_VhVh(my_row_buf0[ci], v_neg_inf);
                HVX_VectorPred q_neg_inf1 = Q6_Q_vcmp_eq_VhVh(my_row_buf1[ci], v_neg_inf);
                v_p_row0_hf = Q6_V_vmux_QVV(q_neg_inf0, v_zero, v_p_row0_hf);
                v_p_row1_hf = Q6_V_vmux_QVV(q_neg_inf1, v_zero, v_p_row1_hf);
...
static __attribute__((noinline)) void fa_ml_update_and_build_d(struct hmx_fa_context * factx,
    ...
    const HVX_Vector v_neg_inf = Q6_Vh_vsplat_R(0xfc00);
    const HVX_Vector v_one_hf  = hvx_vec_splat_f16(1.0f);
    ...
    for (size_t i = 0; i < n_row_vec_cnt; ++i) {
        ...
        HVX_VectorPred q_empty = Q6_Q_vcmp_eq_VhVh(v_m_curr, v_neg_inf);
...
#endif
        v_exp_m_diff = Q6_V_vmux_QVV(q_empty, v_one_hf, v_exp_m_diff);
...
static __attribute__((noinline)) void fa_build_d_diag_inv_l(struct hmx_fa_context * factx,
    ...
    for (size_t i = 0; i < n_row_tiles; ++i) {
        if ((i % 2) == 0) {
            ...
            HVX_Vector     l_lo   = Q6_V_lo_W(vp_l);
            HVX_Vector     l_hi   = Q6_V_hi_W(vp_l);
            HVX_Vector     inv_lo = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(one, hvx_vec_inverse_f32(l_lo)));
            HVX_Vector     inv_hi = Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(one, hvx_vec_inverse_f32(l_hi)));
            inv_lo = Q6_V_vmux_QVV(Q6_Q_vcmp_eq_VwVw(l_lo, zero), zero, inv_lo);
            inv_hi = Q6_V_vmux_QVV(Q6_Q_vcmp_eq_VwVw(l_hi, zero), zero, inv_hi);
...
static __attribute__((noinline)) void fa_compute_slopes(
    ...
    const size_t local_slopes_count = hex_align_up(n_rows_g, VLEN_FP16);
    __fp16 local_slopes[local_slopes_count] __attribute__((aligned(128)));
    memset(local_slopes, 0, local_slopes_count * sizeof(*local_slopes));
    ...
    hvx_copy_f16_aa((uint8_t *)slopes, (const uint8_t *)local_slopes, local_slopes_count);
}
    ...
    for (uint32_t ib3 = 0; ib3 < neq3; ++ib3) {
        for (uint32_t kv_head = 0; kv_head < n_kv_heads; ++kv_head) {
            ...
            for (uint32_t q_start = 0; q_start < neq1; q_start += Br) {
                ...
                hvx_splat_u16_a(factx.vtcm_m_vec,  0xfc00, col_vec_bytes/2);
