...
static int execute_op(struct htp_ops_context * octx) {
    // LORA_ACC_INPLACE does not use VTCM, so it preserves the activation cache.
    if (octx->op != HTP_OP_MUL_MAT && octx->op != HTP_OP_MUL_MAT_ID &&
        octx->op != HTP_OP_LORA_ACC_INPLACE) {
        octx->src1_spad.src    = NULL;
        octx->src1_spad.format = HTP_SPAD_FORMAT_NONE;
    }
    ...
    switch (octx->op) {
        ...
        case HTP_OP_LORA_ACC_INPLACE:
            return op_lora_acc_inplace(octx);
...
static inline bool mmap_buf(struct htp_context *ctx, struct htp_buf_desc *b) {
    ...
    FARF(ERROR, "mmap failed : no free mapping slot for fd %u", b->fd);
    return false;
}

static bool checked_mul_u32(uint32_t lhs, uint32_t rhs, uint32_t *result) {
...
static bool checked_add_u32(uint32_t lhs, uint32_t rhs, uint32_t *result) {
...
static bool validate_opbatch_layout(
        const struct htp_opbatch_req * req,
        const struct dspqueue_buffer * dbuf,
        uint32_t * b_size,
        uint32_t * t_size,
        uint32_t * o_size,
        uint32_t * p_size) {
    if (req->n_bufs == 0 || req->n_bufs > HTP_OP_MAX_BUFS ||
        req->n_tensors == 0 || req->n_tensors > HTP_OP_MAX_TENSORS ||
        req->n_ops == 0 || req->n_ops > HTP_OP_MAX_REQS) {
        FARF(ERROR, "invalid opbatch counts: n-bufs %u n-tensors %u n-ops %u",
             req->n_bufs, req->n_tensors, req->n_ops);
        return false;
    }

    if (req->flags != 0 || req->pad != 0) {
        FARF(ERROR, "invalid opbatch flags 0x%x or padding 0x%x", req->flags, req->pad);
        return false;
    }

    const uint32_t max_tensors = req->n_ops * (HTP_OP_MAX_INPUTS + 1);
    if (req->n_tensors > max_tensors) {
        FARF(ERROR, "invalid opbatch tensor count %u for %u ops", req->n_tensors, req->n_ops);
        return false;
    }

    if (!checked_mul_u32(req->n_bufs, sizeof(struct htp_buf_desc), b_size) ||
        !checked_mul_u32(req->n_tensors, sizeof(struct htp_tensor), t_size) ||
        !checked_mul_u32(req->n_ops, sizeof(struct htp_op_desc), o_size) ||
        !checked_mul_u32(req->n_ops, sizeof(struct htp_prof_desc), p_size)) {
        FARF(ERROR, "opbatch descriptor size overflow");
        return false;
    }

    uint32_t total_size;
    if (!checked_add_u32(*b_size, *t_size, &total_size) ||
        !checked_add_u32(total_size, *o_size, &total_size) ||
        !checked_add_u32(total_size, *p_size, &total_size)) {
        FARF(ERROR, "opbatch memory block size overflow");
        return false;
    }

    if (dbuf->ptr == NULL || ((uintptr_t) dbuf->ptr & (sizeof(uint64_t) - 1)) != 0 || dbuf->size != total_size) {
        FARF(ERROR, "invalid opbatch memory block: ptr %p size %u expected %u",
             dbuf->ptr, dbuf->size, total_size);
        return false;
    }
...
static bool validate_buf_descs(
        const struct htp_context * ctx,
        const struct htp_buf_desc * bufs,
        uint32_t n_bufs) {
    uint64_t total_size = 0;

    for (uint32_t i = 0; i < n_bufs; i++) {
        const struct htp_buf_desc * b = bufs + i;
        if (b->fd == UINT32_MAX || b->size == 0 || b->size > UINT32_MAX ||
            b->size > ctx->max_vmem || b->flags != 0) {
            FARF(ERROR, "invalid buffer #%u: fd %u size %zu flags 0x%x",
                 i, b->fd, (size_t) b->size, b->flags);
            return false;
        }
#if __HVX_ARCH__ <= 73
        if (b->size > HTP_MMAP_MAX_VMEM) {
            FARF(ERROR, "invalid buffer #%u: size %zu exceeds HAP_mmap limit", i, (size_t) b->size);
            return false;
        }
#endif

        if (total_size > ctx->max_vmem - b->size) {
            FARF(ERROR, "opbatch buffer size exceeds max vmem");
            return false;
        }
        total_size += b->size;
        ...
        for (uint32_t j = 0; j < i; j++) {
            if (bufs[j].fd == b->fd) {
                FARF(ERROR, "duplicate buffer fd %u", b->fd);
                return false;
            }
        }

        for (uint32_t j = 0; j < HTP_MAX_MMAPS; j++) {
            const struct htp_mmap * mmap = ctx->mmap + j;
            if (mmap->size != 0 && mmap->fd == b->fd && mmap->size != b->size) {
                FARF(ERROR, "buffer fd %u size mismatch: %zu != %zu", b->fd,
                     (size_t) b->size, (size_t) mmap->size);
                return false;
            }
        }
    }
...
static bool tensor_type_layout(uint16_t type, uint32_t * type_size, uint32_t * block_size) {
    switch (type) {
        case HTP_TYPE_F32:    *type_size = sizeof(float);        *block_size = 1;         return true;
        case HTP_TYPE_F16:    *type_size = sizeof(ggml_half);    *block_size = 1;         return true;
        case HTP_TYPE_Q4_0:   *type_size = sizeof(block_q4_0);   *block_size = QK4_0;     return true;
        case HTP_TYPE_Q4_1:   *type_size = sizeof(block_q4_1);   *block_size = QK4_1;     return true;
        case HTP_TYPE_Q8_0:   *type_size = sizeof(block_q8_0);   *block_size = QK8_0;     return true;
        case HTP_TYPE_IQ4_NL: *type_size = sizeof(block_iq4_nl); *block_size = QK4_NL;    return true;
        case HTP_TYPE_I32:    *type_size = sizeof(int32_t);      *block_size = 1;         return true;
        case HTP_TYPE_I64:    *type_size = sizeof(int64_t);      *block_size = 1;         return true;
        case HTP_TYPE_MXFP4:  *type_size = sizeof(block_mxfp4);  *block_size = QK_MXFP4;  return true;
        case HTP_TYPE_Q4_0x4x2:
            *type_size = QK_Q4_0x4x2 / 2 + 16;
            *block_size = QK_Q4_0x4x2;
            return true;
        case HTP_TYPE_Q4_1x4x2:
            *type_size = QK_Q4_0x4x2 / 2 + 32;
            *block_size = QK_Q4_0x4x2;
            return true;
        case HTP_TYPE_Q8_0x4x2:
            *type_size = QK_Q8_0x4x2 + 16;
            *block_size = QK_Q8_0x4x2;
            return true;
        case HTP_TYPE_MXFP4x4x2:
            *type_size = QK_MXFP4x4x2 / 2 + 8;
            *block_size = QK_MXFP4x4x2;
            return true;
        case HTP_TYPE_INVALID:
            break;
    }
...
static bool add_tensor_span(uint64_t * span, uint64_t count, uint64_t stride, uint64_t limit) {
    if (*span > limit || (count != 0 && stride > (limit - *span) / count)) {
        return false;
    }
...
static bool validate_tensor_desc(
        const struct htp_tensor * tensor,
        const struct htp_buf_desc * bufs,
        uint32_t n_bufs,
        uint32_t index) {
    if (tensor->bi >= n_bufs || tensor->size == 0) {
        FARF(ERROR, "invalid tensor #%u: bi %u size %u", index, tensor->bi, tensor->size);
        return false;
    }

    const uint32_t valid_flags = HTP_TENSOR_COMPUTE | HTP_TENSOR_FLUSHED | HTP_TENSOR_HOST_DIRTY;
    if ((tensor->flags & ~valid_flags) != 0) {
        FARF(ERROR, "invalid tensor #%u flags 0x%x", index, tensor->flags);
        return false;
    }

    const struct htp_buf_desc * buffer = bufs + tensor->bi;
    if ((uint64_t) tensor->data > buffer->size || tensor->size > buffer->size - tensor->data) {
        FARF(ERROR, "tensor #%u range exceeds buffer #%u", index, tensor->bi);
        return false;
    }

    uint32_t type_size;
    uint32_t block_size;
    if (!tensor_type_layout(tensor->type, &type_size, &block_size) || tensor->nb[0] < type_size) {
        FARF(ERROR, "invalid tensor #%u type %u or element stride %u", index, tensor->type, tensor->nb[0]);
        return false;
    }

    for (uint32_t dim = 0; dim < HTP_OP_MAX_DIMS; dim++) {
        if (tensor->ne[dim] == 0 || tensor->nb[dim] == 0) {
            FARF(ERROR, "invalid tensor #%u shape or stride at dim %u", index, dim);
            return false;
        }
    }
    ...
    if (block_size == 1) {
        span = type_size;
        if (!add_tensor_span(&span, tensor->ne[0] - 1, tensor->nb[0], tensor->size)) {
            FARF(ERROR, "tensor #%u dim-0 span exceeds tensor size", index);
            return false;
        }
    } else {
        if (tensor->ne[0] % block_size != 0) {
            FARF(ERROR, "tensor #%u dim-0 %u is not block aligned", index, tensor->ne[0]);
            return false;
        }

        const uint64_t blocks = tensor->ne[0] / block_size;
        if (blocks > tensor->size / tensor->nb[0]) {
            FARF(ERROR, "tensor #%u dim-0 span exceeds tensor size", index);
            return false;
        }
        span = blocks * tensor->nb[0];
    }

    for (uint32_t dim = 1; dim < HTP_OP_MAX_DIMS; dim++) {
        if (!add_tensor_span(&span, tensor->ne[dim] - 1, tensor->nb[dim], tensor->size)) {
            FARF(ERROR, "tensor #%u span exceeds tensor size at dim %u", index, dim);
            return false;
        }
    }
...
static bool validate_op_descs(const struct htp_op_desc * ops, uint32_t n_ops, uint32_t n_tensors) {
    for (uint32_t i = 0; i < n_ops; i++) {
        const struct htp_op_desc * op = ops + i;
        if (op->opcode >= HTP_OP_INVALID || op->dst >= n_tensors ||
            (op->flags & ~HTP_OPFLAGS_SKIP_COMPUTE) != 0) {
            FARF(ERROR, "invalid op #%u: opcode %u flags 0x%x dst %u",
                 i, op->opcode, op->flags, op->dst);
            return false;
        }

        for (uint32_t j = 0; j < HTP_OP_MAX_INPUTS; j++) {
            if (op->src[j] != UINT16_MAX && op->src[j] >= n_tensors) {
                FARF(ERROR, "invalid op #%u src #%u tensor index %u", i, j, op->src[j]);
                return false;
            }
        }
    }
...
static bool prep_op_bufs(struct htp_context *ctx, struct htp_buf_desc *bufs, uint32_t n_bufs) {
    ...
    for (uint32_t i=0; i < n_bufs; i++) {
        const struct htp_buf_desc *b = bufs + i;
        const uint64_t address_space = (uint64_t) UINT32_MAX + 1;
        if (b->base == 0 || b->base >= address_space || b->size > address_space - b->base) {
            FARF(ERROR, "mapped buffer #%u exceeds 32-bit address space", i);
            return false;
        }
    }
...
static int proc_op_req(struct htp_ops_context * octx, struct htp_tensor *tens, uint32_t idx, struct htp_op_desc * op) {
    ...
    for (uint32_t i=0; i<HTP_OP_MAX_INPUTS; i++) {
        ...
        if (src->flags & HTP_TENSOR_HOST_DIRTY) {
            hex_l2invalidate((void *) src->data, src->size);
            src->flags &= ~HTP_TENSOR_HOST_DIRTY;
        } else if (!(src->flags & HTP_TENSOR_FLUSHED) && (src->flags & HTP_TENSOR_COMPUTE)) {
    ...
    const int status = execute_op(octx);
    if (!htp_status_is_ok(status)) {
        FARF(ERROR, "op #%u failed: opcode %u status %d", idx, octx->op, status);
        return status > 0 ? status : HTP_STATUS_INTERNAL_ERR;
    }
...
static int write_opbatch_response(
        dspqueue_t queue,
        ...
        uint32_t n_tensors,
        uint32_t n_ops) {
    const struct htp_opbatch_rsp rsp = {
        .id = id,
        .status = status,
        .n_bufs = n_bufs,
        .n_tensors = n_tensors,
    ...
    return dspqueue_write(queue, 0, 1, dbuf, sizeof(rsp), (const uint8_t *) &rsp, DSPQUEUE_TIMEOUT_NONE);
}
...
static void htp_packet_callback(dspqueue_t queue, int error, void * context) {
    ...
    while (!ctx->vtcm_needs_release) {
        struct htp_opbatch_req req = {0};
        uint32_t r_size = sizeof(req);

        struct dspqueue_buffer dbuf = {0};
        uint32_t n_dbufs = 1;
        uint32_t flags   = 0;
        ...
        if (r_size != sizeof(req) || n_dbufs != 1) {
            ...
            if (n_dbufs == 1) {
                err = write_opbatch_response(queue, &dbuf, req.id, HTP_STATUS_INVAL_PARAMS, 0, 0, 0);
                if (err != 0) {
        ...
        uint32_t o_size = 0;
        uint32_t p_size = 0;
        uint32_t rsp_status = HTP_STATUS_INVAL_PARAMS;

        do {
            if (!validate_opbatch_layout(&req, &dbuf, &b_size, &t_size, &o_size, &p_size)) {
                break;
            }
            ...
            if (!validate_buf_descs(ctx, bufs, n_bufs)) {
                break;
            }

            bool tensors_valid = true;
            for (uint32_t i = 0; i < n_tens; i++) {
                if (!validate_tensor_desc(tens + i, bufs, n_bufs, i)) {
                    tensors_valid = false;
                    break;
                }
            }
            if (!tensors_valid || !validate_op_descs(ops, n_ops, n_tens)) {
                break;
            }

            if (!prep_op_bufs(ctx, bufs, n_bufs)) {
            ...
            for (uint32_t i=0; i < n_ops; i++) {
                ...
                if (!htp_status_is_ok(op_status)) {
                    rsp_status = op_status > 0 ? op_status : HTP_STATUS_INTERNAL_ERR;
                    break;
                }
            }
        } while (0);

        err = write_opbatch_response(queue, &dbuf, req.id, rsp_status, n_bufs, n_tens, n_ops);
        if (err != 0) {
            FARF(ERROR, "dspqueue_write failed: 0x%08x", (unsigned) err);
            break;
        }
