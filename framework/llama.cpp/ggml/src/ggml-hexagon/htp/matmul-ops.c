...
static int op_matmul_hvx(struct htp_ops_context * octx) {
    ...
    enum htp_spad_format src1_spad_format = HTP_SPAD_FORMAT_NONE;
    ...
    if (src0->type == HTP_TYPE_F16) {
        ...
        if (!is_batched && !is_permuted && f16_total_size <= octx->ctx->vtcm_size) {
            ...
            src1_spad_format   = HTP_SPAD_FORMAT_F16;
            mmctx->type        = "f16-f16";
        ...
        if (!is_batched && !is_permuted && f32_total_size <= octx->ctx->vtcm_size) {
            ...
            src1_spad_format   = HTP_SPAD_FORMAT_F32;
            mmctx->type        = "f32-f32";
        ...
        if (src0->type == HTP_TYPE_Q4_1) {
            ...
            src1_spad_format = HTP_SPAD_FORMAT_Q8_1X4X2;
        } else {
            quant_job_func = quantize_f32_q8x4x2;
            src1_row_size  = q8x4x2_row_size(ne10);
            src1_spad_format = HTP_SPAD_FORMAT_Q8_0X4X2;
        }
    ...
    const bool reuse_src1_spad = need_quant &&
        src1 == octx->src1_spad.src &&
        src1_spad_format == octx->src1_spad.format &&
        src1_row_size == octx->src1_spad.stride;

    octx->src1_spad.src    = reuse_src1_spad ? src1 : NULL;
    octx->src1_spad.format = reuse_src1_spad ? src1_spad_format : HTP_SPAD_FORMAT_NONE;
    ...
    if (need_quant && !octx->src1_spad.src) {
        ...
        octx->src1_spad.format = src1_spad_format;
    }
...
int op_matmul(struct htp_ops_context * octx) {
    ...
    if (!hex_is_aligned((const void *) (uintptr_t) src0->data, VLEN) ||
        !hex_is_aligned((const void *) (uintptr_t) src1->data, VLEN) ||
        !hex_is_aligned((const void *) (uintptr_t) dst->data, VLEN)) {
        return op_matmul_hvx(octx);
    }
    ...
    int ret = HTP_STATUS_INTERNAL_ERR;
    ...
    if (!htp_status_is_ok(ret)) {
        if (ret == HTP_STATUS_NO_SUPPORT || ret == HTP_STATUS_VTCM_TOO_SMALL) {
            static bool reported_hmx_fallback = false;
            if (!reported_hmx_fallback) {
                FARF(ALWAYS, "HMX matmul unavailable (status=%d); using HVX", ret);
                reported_hmx_fallback = true;
            }
            return op_matmul_hvx(octx);
        }
        FARF(ERROR, "HMX matmul failed (status=%d)", ret);
        return ret;
    }

    static bool reported_hmx_active = false;
    if (!reported_hmx_active) {
        FARF(ALWAYS, "HMX matmul active");
        reported_hmx_active = true;
    }
...
int op_matmul_id(struct htp_ops_context * octx) {
    ...
    enum htp_spad_format src1_spad_format;
    ...
    if (hmx_eligible) {
        ...
        for (uint32_t cur_a = 0; cur_a < n_as; ++cur_a) {
            ...
            if (!htp_status_is_ok(ret)) {
                if (ret == HTP_STATUS_NO_SUPPORT || ret == HTP_STATUS_VTCM_TOO_SMALL) {
                    static bool reported_hmx_id_fallback = false;
                    if (!reported_hmx_id_fallback) {
                        FARF(ALWAYS, "HMX matmul-id unavailable (status=%d); using HVX", ret);
                        reported_hmx_id_fallback = true;
                    }
                    hmx_fallback = true;
                    break;
                }
                FARF(ERROR, "HMX matmul-id failed for expert %u, status %d", cur_a, ret);
                if (must_free_mapping) free(mapping_buf);
                return ret;
            }
        }
        ...
        if (!hmx_fallback) {
            static bool reported_hmx_id_active = false;
            if (!reported_hmx_id_active) {
                FARF(ALWAYS, "HMX matmul-id active");
                reported_hmx_id_active = true;
            }
            if (must_free_mapping) free(mapping_buf);
            return HTP_STATUS_OK;
        }
