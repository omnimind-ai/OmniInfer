...
static void ggml_compute_forward_lora_acc_inplace(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
    const struct ggml_tensor * tmp      = dst->src[1];
    const struct ggml_tensor * b_pair   = dst->src[2];
    const struct ggml_tensor * row_side = dst->src[3];

    struct ggml_lora_acc_params op_params = { 0 };
    memcpy(&op_params, dst->op_params, sizeof(op_params));

    GGML_ASSERT(dst->type == GGML_TYPE_F32);
    GGML_ASSERT(tmp->type == GGML_TYPE_F32);
    GGML_ASSERT(b_pair->type == GGML_TYPE_F16);
    GGML_ASSERT(row_side->type == GGML_TYPE_I32);
    GGML_ASSERT((op_params.flags & ~(GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK |
    ...
    if (params->ith == 0) {
        int32_t previous_side = 0;
        for (int64_t im = 0; im < m; ++im) {
            const int32_t side = *(const int32_t *) ((const char *) row_side->data + im*row_side->nb[0]);
            GGML_ASSERT(side == 0 || side == 1);
            if (op_params.flags & GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS) {
                GGML_ASSERT(side >= previous_side);
                previous_side = side;
            }
        }
    }
    ...
    for (int64_t index = params->ith; index < n*m; index += params->nth) {
        const int64_t im = index / n;
        const int64_t in = index % n;
        const int32_t side = *(const int32_t *) ((const char *) row_side->data + im*row_side->nb[0]);

        float delta = 0.0f;
        for (int64_t ir = 0; ir < rank; ++ir) {
            const ggml_fp16_t b = *(const ggml_fp16_t *) ((const char *) b_pair->data +
                    side*b_pair->nb[2] + ir*b_pair->nb[1] + in*b_pair->nb[0]);
            const float coeff = *(const float *) ((const char *) tmp->data + im*tmp->nb[1] + ir*tmp->nb[0]);
            delta += GGML_CPU_FP16_TO_FP32(b) * coeff;
        }

        float * out = (float *) ((char *) dst->data + im*dst->nb[1] + in*dst->nb[0]);
        *out += op_params.scale * delta;
    }
}
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_LORA_ACC_INPLACE:
            {
                ggml_compute_forward_lora_acc_inplace(params, tensor);
            } break;
