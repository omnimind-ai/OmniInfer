...
struct ggml_tensor * ggml_lora_acc_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * base,
        struct ggml_tensor  * tmp,
        struct ggml_tensor  * b_pair,
        struct ggml_tensor  * row_side,
        float                 scale,
        int32_t               flags) {
    ...
    GGML_ASSERT(base->ne[2] == 1 && base->ne[3] == 1);
    GGML_ASSERT(tmp->ne[2] == 1 && tmp->ne[3] == 1);
    GGML_ASSERT(b_pair->ne[2] == 2 && b_pair->ne[3] == 1);
    GGML_ASSERT(row_side->ne[0] == base->ne[1]);
    GGML_ASSERT(ggml_nelements(row_side) == base->ne[1]);
    GGML_ASSERT(tmp->ne[1] == base->ne[1]);
    GGML_ASSERT(b_pair->ne[0] == ((base->ne[0] + 63)/64)*64);
    GGML_ASSERT(b_pair->ne[1] == tmp->ne[0]);
    GGML_ASSERT(tmp->ne[0] == 8 || tmp->ne[0] == 16 || tmp->ne[0] == 24 || tmp->ne[0] == 32);
    GGML_ASSERT(ggml_is_contiguous(base));
    GGML_ASSERT(ggml_is_contiguous(tmp));
    GGML_ASSERT(ggml_is_contiguous(b_pair));
    GGML_ASSERT(ggml_is_contiguous(row_side));
    GGML_ASSERT((flags & ~(GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK |
                           GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS)) == 0);

    struct ggml_tensor * result = ggml_view_tensor(ctx, base);

    result->op     = GGML_OP_LORA_ACC_INPLACE;
    result->src[0] = base;
    result->src[1] = tmp;
    result->src[2] = b_pair;
    result->src[3] = row_side;

    const struct ggml_lora_acc_params params = {
        /* .scale = */ scale,
        /* .flags = */ flags,
    };
    ggml_set_op_params(result, &params, sizeof(params));
