...
struct test_case {
    ...
    virtual bool supports_cpu_reference_op(const ggml_tensor * t) {
        GGML_UNUSED(t);
        return false;
    }
    ...
    test_status_t eval(ggml_backend_t backend1,
        ...
        for (ggml_backend_t backend : {backend1, backend2}) {
            for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
                ...
                if (!ggml_backend_supports_op(backend, t) && !(is_cpu && supports_cpu_reference_op(t))) {
                    supported = false;
                    break;
                }
            }
...
// GGML_OP_LORA_ACC_INPLACE
struct test_lora_acc_inplace : public test_case {
    static constexpr int64_t n = 96;
    static constexpr int64_t m = 18;
    static constexpr int64_t split = 9;
    static constexpr float scale = 0.75f;

    const int64_t rank;

    std::string vars() override {
        return VARS_TO_STR4(n, m, rank, split);
    }

    test_lora_acc_inplace(int64_t rank) : rank(rank) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        const int64_t padded_n = GGML_PAD(n, 64);
        const int32_t flags = GGML_LORA_ACC_FLAG_NO_CPU_FALLBACK |
                              GGML_LORA_ACC_FLAG_SIDE_CONTIGUOUS;

        ggml_tensor * base = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n, m);
        ggml_tensor * tmp = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, rank, m);
        ggml_tensor * b_pair = ggml_new_tensor_3d(ctx, GGML_TYPE_F16, padded_n, rank, 2);
        ggml_tensor * row_side = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, m);
        ggml_tensor * expected = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n, m);
        ggml_set_name(base, "lora_base");
        ggml_set_name(tmp, "lora_tmp");
        ggml_set_name(b_pair, "lora_b_pair");
        ggml_set_name(row_side, "lora_row_side");
        ggml_set_name(expected, "lora_expected");

        ggml_tensor * base_plus = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n, split);
        ggml_tensor * tmp_plus = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, rank, split);
        ggml_tensor * side_plus = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, split);
        ggml_set_name(base_plus, "lora_base_plus");
        ggml_set_name(tmp_plus, "lora_tmp_plus");
        ggml_set_name(side_plus, "lora_side_plus");

        ggml_tensor * base_minus = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n, m - split);
        ggml_tensor * tmp_minus = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, rank, m - split);
        ggml_tensor * side_minus = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, m - split);
        ggml_set_name(base_minus, "lora_base_minus");
        ggml_set_name(tmp_minus, "lora_tmp_minus");
        ggml_set_name(side_minus, "lora_side_minus");

        ggml_tensor * paired = ggml_lora_acc_inplace(ctx, base, tmp, b_pair, row_side, scale, flags);
        ggml_tensor * plus = ggml_lora_acc_inplace(ctx, base_plus, tmp_plus, b_pair, side_plus, scale, flags);
        ggml_tensor * minus = ggml_lora_acc_inplace(ctx, base_minus, tmp_minus, b_pair, side_minus, scale, flags);
        ggml_set_name(paired, "lora_paired");
        ggml_set_name(plus, "lora_plus");
        ggml_set_name(minus, "lora_minus");

        ggml_tensor * serial = ggml_concat(ctx, plus, minus, 1);
        ggml_tensor * reference_error = ggml_sub(ctx, paired, expected);
        ggml_tensor * serial_error = ggml_sub(ctx, paired, serial);
        ggml_tensor * out = ggml_concat(ctx, reference_error, serial_error, 1);
        ggml_set_name(out, "out");
        return out;
    }
    ...
    void initialize_tensors(ggml_context * ctx) override {
        const int64_t padded_n = GGML_PAD(n, 64);
        std::vector<float> base((size_t) n*m);
        std::vector<float> tmp((size_t) rank*m);
        std::vector<ggml_fp16_t> b_pair((size_t) padded_n*rank*2);
        std::vector<int32_t> row_side(m);
        std::vector<float> expected((size_t) n*m);

        for (int64_t im = 0; im < m; ++im) {
            row_side[im] = im < split ? 0 : 1;
            for (int64_t in = 0; in < n; ++in) {
                base[(size_t) im*n + in] = (float) ((im*7 + in*3) % 23 - 11) / 16.0f;
            }
            for (int64_t ir = 0; ir < rank; ++ir) {
                tmp[(size_t) im*rank + ir] = (float) ((im*5 + ir*7) % 17 - 8) / 32.0f;
            }
        }

        for (int64_t side = 0; side < 2; ++side) {
            for (int64_t ir = 0; ir < rank; ++ir) {
                for (int64_t in = 0; in < padded_n; ++in) {
                    const float value = (float) (((side + 1)*11 + ir*5 + in*3) % 29 - 14) / 64.0f;
                    b_pair[((size_t) side*rank + ir)*padded_n + in] = ggml_fp32_to_fp16(value);
                }
            }
        }

        for (int64_t im = 0; im < m; ++im) {
            const int32_t side = row_side[im];
            for (int64_t in = 0; in < n; ++in) {
                float delta = 0.0f;
                for (int64_t ir = 0; ir < rank; ++ir) {
                    const ggml_fp16_t value = b_pair[((size_t) side*rank + ir)*padded_n + in];
                    delta += ggml_fp16_to_fp32(value) * tmp[(size_t) im*rank + ir];
                }
                expected[(size_t) im*n + in] = base[(size_t) im*n + in] + scale*delta;
            }
        }

        std::vector<float> base_plus(base.begin(), base.begin() + split*n);
        std::vector<float> tmp_plus(tmp.begin(), tmp.begin() + split*rank);
        std::vector<int32_t> side_plus(split, 0);
        std::vector<float> base_minus(base.begin() + split*n, base.end());
        std::vector<float> tmp_minus(tmp.begin() + split*rank, tmp.end());
        std::vector<int32_t> side_minus(m - split, 1);

        auto set_tensor = [&](const char * name, const void * data, size_t size) {
        ...
        set_tensor("lora_base", base.data(), base.size()*sizeof(float));
        set_tensor("lora_tmp", tmp.data(), tmp.size()*sizeof(float));
        set_tensor("lora_b_pair", b_pair.data(), b_pair.size()*sizeof(ggml_fp16_t));
        set_tensor("lora_row_side", row_side.data(), row_side.size()*sizeof(int32_t));
        set_tensor("lora_expected", expected.data(), expected.size()*sizeof(float));
        set_tensor("lora_base_plus", base_plus.data(), base_plus.size()*sizeof(float));
        set_tensor("lora_tmp_plus", tmp_plus.data(), tmp_plus.size()*sizeof(float));
        set_tensor("lora_side_plus", side_plus.data(), side_plus.size()*sizeof(int32_t));
        set_tensor("lora_base_minus", base_minus.data(), base_minus.size()*sizeof(float));
        set_tensor("lora_tmp_minus", tmp_minus.data(), tmp_minus.size()*sizeof(float));
        set_tensor("lora_side_minus", side_minus.data(), side_minus.size()*sizeof(int32_t));
    }

    double err(const float * a, const float * b, size_t count) override {
        double maximum = 0.0;
        for (size_t i = 0; i < count; ++i) {
            maximum = std::max(maximum, (double) std::max(std::abs(a[i]), std::abs(b[i])));
        }
        return maximum;
    }
    ...
    bool supports_cpu_reference_op(const ggml_tensor * t) override {
        return t->op == GGML_OP_LORA_ACC_INPLACE;
    }

    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return ggml_op_name(GGML_OP_LORA_ACC_INPLACE);
    }
};
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    for (int64_t rank : {8, 16, 24, 32}) {
        test_cases.emplace_back(new test_lora_acc_inplace(rank));
    }
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_flash_attn_ext(64, 64, 4, {2, 3}, 65, 5, true, false, 0, 0,
        ...
        if (ctx) {
            ...
            if (out && out->op != GGML_OP_NONE) {
                ...
                if (out->op == GGML_OP_UNARY) {
                    graph_op = ggml_unary_op_name(ggml_get_unary_op(out));
                } else if (out->op == GGML_OP_GLU) {
                    graph_op = ggml_glu_op_name(ggml_get_glu_op(out));
                } else {
                    graph_op = ggml_op_name(out->op);
                }

                const std::string described_op = test_case->op_desc(out);
                if (!test_case->run_whole_graph()) {
                    tested_ops.insert(graph_op);
                }
                if (!test_case->run_whole_graph() || described_op != graph_op) {
                    tested_ops.insert(described_op);
                }
            }
            ggml_free(ctx);
        }
