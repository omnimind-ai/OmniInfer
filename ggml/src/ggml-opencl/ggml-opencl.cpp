    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MUL_MAT:
            if (!any_on_device && !ggml_cl_can_mul_mat(tensor->src[0], tensor->src[1], tensor)) {
                fprintf(stderr, "%s: mul_mat not running on OpenCL!\n", __func__);
                return false;
            }

            // {
            //     struct ggml_tensor * s0 = tensor->src[0];
            //     fprintf(stderr, "%s: OpenCL mul_mat OK, dst %s, type %s, shape (%ld,%ld,%ld,%ld); src0 shape (%ld,%ld,%ld,%ld)\n", __func__, tensor->name,
            //         ggml_type_name(tensor->type), tensor->ne[0], tensor->ne[1], tensor->ne[2], tensor->ne[3],
