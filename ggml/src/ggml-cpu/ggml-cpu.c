...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        // int64_t time_ms = ggml_time_ms() - t0;
        // if (state->ith == 0) {
        //     fprintf(stderr, "node %s took %ld ms, op %s shape %d %d %d %d\n", node->name, time_ms, ggml_op_name(node->op), node->ne[0], node->ne[1], node->ne[2], node->ne[3]);
        // }

        // if (state->ith == 0) {
        //     float *d = (float *)node->data;
        //     fprintf(stderr, "node %s [op %s] : %f %f %f %f\n", node->name, ggml_op_name(node->op),
        //             d[0], d[1], d[2], d[3]);
        // }

        // dump_logic
        // if (state->ith == 0 && strstr(node->name, "ffn_out_")) {
        //     static char path[128];
        //     snprintf(path, sizeof(path), "cpu-dump/%s.bin", node->name);

        //     FILE * f = fopen(path, "wb");
        //     GGML_ASSERT(f);
        //     fwrite(node->data, ggml_nbytes(node), 1, f);
        //     fclose(f);
        // }

        // // dynamic interference
        // if (state->ith == 0 && strstr(node->name, "ffn_out_") && node->ne[0] == 1280) {
        //     printf("manual fix channel 849...\n");
        //     float *d = (float *) node->data;
            
        //     float sum = 0.0f;
        //     // compute the mean value of channel 849
        //     for (int i = 0; i < ggml_nrows(node); i++)
        //         sum += d[i * node->ne[0] + 849];
        //     float mean = sum / ggml_nrows(node);

        //     // manually set massive outliers in channel 849 to the mean value
        //     for (int i = 0; i < ggml_nrows(node); i++) {
        //         int idx = i * node->ne[0] + 849;
        //         if (fabsf(d[idx]) > 100.0f) {
        //             d[idx] = mean;
        //         }
        //     }
        // }
