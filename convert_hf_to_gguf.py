...
QK8_0 = 32
# ggml_half (FP16) 的字节大小
GGML_HALF_SIZE = 2
# block_q8_0 结构体的总大小 (2 + 32 = 34 字节)
...
def repack_q8_0_super_block_hvx_bytes(src_bytes: bytes) -> bytes:
    """
    【核心函数】将 q8_0 类型的量化块（bytes）重新打包为 HVX 友好的格式。
    这个函数处理原始的 bytes 输入和输出。

    Args:
        src_bytes: 原始的二进制数据，由一系列 block_q8_0 结构组成。

    Returns:
        重新打包后的二进制数据 (bytes)。
    """
    # 一个超级块由 8 个 block_q8_0 组成
    super_block_size = BLOCK_Q8_0_SIZE * 8  # 34 * 8 = 272 字节

    if len(src_bytes) % super_block_size != 0:
        raise ValueError(f"输入数据的大小 ({len(src_bytes)}) 必须是超级块大小 ({super_block_size} 字节) 的整数倍。")
    ...
    # 定义解包格式
    block_format = f"<e{QK8_0}b"  # '<' 小端, 'e' float16, 'b' int8
            ...
            for new_name, data_torch in (self.modify_tensors(data_torch, name, bid)):
                ...
                try:
                    ...
                    if os.getenv("REPACK_FOR_HVX") is not None and data_qtype == gguf.GGMLQuantizationType.Q8_0:
                        # Repack the data to HVX-friendly format
                        logger.warning(f"Repacking Q8_0 data for HVX... {name}")
                        shape = data.shape
...
class Qwen3VLVisionModel(MmprojModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.startswith("visual."):
            if os.getenv("REPACK_FOR_HMX") is not None and data_torch.ndim == 2 and \
                "pos_embed" not in name: # skip pos_embed manually
                data_torch = repack_linear_weight_for_hmx(data_torch)
