...
int32_t mtmd_helper_eval_chunk_single(mtmd_context * ctx,
    ...
    if (chunk_type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        ...
        while (i < n_tokens) { // split into batches
            ...
            LOG_INF("decoding text batch, microbatch n_tokens = %d, total n_tokens = %zu\n",
        ...
        // {
        //     std::vector<float> & embd_v = mtmd_get_output_embd_vector(ctx);

        //     FILE * fp = fopen("embd.bin", "wb");
        //     GGML_ASSERT(fp != nullptr && "failed to open embd.bin for writing");
        //     size_t n_write = fwrite(embd, sizeof(float), embd_v.size(), fp);
        //     fclose(fp);
        //     printf("Wrote %zu floats to embd.bin\n", n_write);
        // }
