...
struct clip_graph {
    ...
    ggml_cgraph * build_qwen2vl() {
        ...
        if (use_window_attn) {
            ...
            const auto n_pos_pad = GGML_PAD(n_pos, GGML_KQ_MASK_PAD);
            // window_mask = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_pos, n_pos_pad);
            window_mask = ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, n_pos_pad, n_pos_pad);
...
private:
    ...
    void cb(ggml_tensor * cur0, const char * name, int il) const {
        ...
        ggml_set_name(cur0, cur_name.c_str());
        if (ctx->debug_graph) {
    ...
    ggml_tensor * build_attn(
        ...
        const bool use_flash_attn = ctx->flash_attn_type == CLIP_FLASH_ATTN_TYPE_ENABLED;
        ...
        if (!use_flash_attn) {
            ...
            if (k_cur->type == GGML_TYPE_F32) {
                k_cur = ggml_cast(ctx0, k_cur, GGML_TYPE_F16);
            }
            if (v_cur->type == GGML_TYPE_F32) {
                v_cur = ggml_cast(ctx0, v_cur, GGML_TYPE_F16);
            }
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    fprintf(stderr, "HZX@%s: image width=%d height=%d, patch_size=%d n_patches=%d\n", __func__, image_size_width, image_size_height, patch_size, num_patches);
    ...
    auto set_input_f16 = [&get_inp_tensor](const char * name, std::vector<uint16_t> & values) {
    ...
    auto set_input_raw_time_ms = ggml_time_ms() - t0;
    fprintf(stderr, "HZX@%s: set_input_raw_time=%ld ms\n", __func__, set_input_raw_time_ms);
        ...
        case PROJECTOR_TYPE_QWEN25VL:
            {
                ...
                if (use_window_attn) {
                    ...
                    const auto n_vis_tokens = ipw * iph;
                    const auto n_tokens_pad = GGML_PAD(n_vis_tokens, GGML_KQ_MASK_PAD);
                    // std::vector<float> mask(n_vis_tokens * GGML_PAD(n_vis_tokens, GGML_KQ_MASK_PAD), std::numeric_limits<float>::lowest());
                    std::vector<uint16_t> mask(n_tokens_pad * n_tokens_pad, 0xfc00);
                    ...
                    for (int y = 0; y < ph; y += grid_window) {
                        for (int x = 0; x < pw; x += grid_window) {
                            ...
                            for (int r=0; r < win_h * win_w * merge_ratio * merge_ratio; r++) {
                                int row_offset = mask_row * n_tokens_pad;
                                std::fill(
                    ...
                    set_input_f16("window_mask",    mask);
                } else {
    ...
    auto set_input_pos_time_ms = ggml_time_ms() - t1;
    fprintf(stderr, "HZX@%s: set_input_pos_time=%ld ms\n", __func__, set_input_pos_time_ms);
